...
public class GUI extends JFrame implements KeyListener {
	...
	private void init_temp_point() {
		...
		for (int i = 0; i < point.length; i++) {
			...
			temp_point[i] = new Points(point[i].a, point[i].q, point[i].w, point[i].d, point[i].e, point[i].x,
					point[i].z, point[i].c, point[i].goal, point[i].adj, point[i].value);
		}
	}
	...
	/*
	 * private void NoPossibleAction(int i,int adj,int flag) { if(i%9 == 8 ||
	 * i%9 == 0||(90< i && i<93 )|| (95<i && i<98)||(0<i && i< 3 )|| (5<i &&
	 * i<9)) if(adj==5) System.out.println("there is no more action.player"
	 * +(flag+1)+"win");
	 * 
	 * else if(i==3 ||i==5 ||i==93 ||i==95) if(adj==3) System.out.println(
	 * "there is no more action.player "+(flag+1)+"win");
	 * 
	 * else if(i==0 ||i==100||i==90 ||i==98 ||i==103 ) if(adj==7)
	 * System.out.println("there is no more action.player "+(flag+1)+"win");
	 * else if(adj==8) System.out.println("there is no more action.player "
	 * +(flag+1)+"win"); }
	 */
	...
	public void SouthWest() {
		...
		if (point[pointNum].z) {
			if (pointNum != 95 && pointNum != 94) {
				...
				if (point[pointNum + 8].adj == 7 & flag == 0) {
					init_temp_point();
					action = minimax.minimax_decision(pointNum, temp_point);
					Act(action);
				} else {
	...
	public void SouthEast() {
		...
		if (point[pointNum].c) {
			if (pointNum != 93 && pointNum != 94) {
				if (point[pointNum + 10].adj == 7 & flag == 0) {
					init_temp_point();
					action = minimax.minimax_decision(pointNum, temp_point);
					Act(action);
				} else {
	...
	public void South() {
		...
		if (point[pointNum].x) {
			if (pointNum != 94) {
				if (point[pointNum + 9].adj == 7 & flag == 0) {
					init_temp_point();
					action = minimax.minimax_decision(pointNum, temp_point);
					Act(action);
				} else {
	...
	public void West() {
		...
		if (point[pointNum].a) {
			...
			if (point[pointNum - 1].adj == 7 & flag == 0) {
				init_temp_point();
				action = minimax.minimax_decision(pointNum, temp_point);
				Act(action);
			} else {
